...
export default function SideNavLeft() {
  ...
  const Menus = [
    ...
    { title: "Profile", spacing: true, icon: <BookOpen /> },
    { title: "Logout", icon: <LogOut /> },
  ];

  return (
    <div className={`bg-[var(--sidebar)] h-screen p-5 pt-8
        ${open ? "w-72" : "w-20"} duration-300 relative`}>
      <ArrowLeft className={`bg-white text-[var(--sidebar-foreground)]
        text-3xl rounded-full absolute -right-3 top-9 border
        border-[var(--sidebar-border)] cursor-pointer ${!open && "rotate-180"}`}
        onClick={() => setOpen(!open)} />

      <div className="inline-flex p-2">
        <House className={`bg-amber300 text-4xl rounded cursor-pointer block
          float-left mt-1 mr-1 duration-300 ${open && "rotate-[360deg]"}`} />
        <h1 className={`text-black origin-left font-medium
          text-2xl duration-300 ${!open && "scale-0"}`}>Home</h1>
      </div>
          ...
          <div key={index}>
            <li
              className={`text-black text-sm flex items-center gap-x-4
                cursor-pointer p-2 hover:bg-[var(--sidebar-accent)]
                ...
                {menu.subMenuItems.map((item, idx) => (
                  <li key={idx}
                    className="text-black text-sm flex items-center gap-x-4 cursor-pointer
                      p-2 px-5 hover:bg-[var(--sidebar-accent)]">
                    {item.title}
                  </li>
                ))}
              </ul>
